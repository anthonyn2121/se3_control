...
class SE3Control(object):
    ...
    def update(self, dt, state, desired_state):
        ...
        # vector in inertial frame indicating where nose of quadrotor should point -- nose aligns with desired path
        b1d = np.array([1, 0, 0])

        b3d = -(self.Kx*ex) - (self.Kv*ev) - (self.mass*self.g *
                                              self.e3) + (self.mass*desired_state['x_ddot'])
        ...
        # desired attitude as a orthogonal group in SO(3)
        Rd = np.vstack((b1d, b2d, b3d))
        print("Rd: \n", Rd)
        ...
        # desired angular velocity is 0, so the equation (omega - R @ Rd @ Omegad) == 0
        eOmega = state['omega']

        # Find control inputs and update dynamics
        f = (-self.Kx*ex - self.Kv*ev - self.mass*self.g*self.e3) + \
            self.mass*desired_state['x_ddot']  # total thrust
        M = -self.KR*eR - self.KOmega*eOmega + \
            np.cross(state['omega'], self.inertia *
                     state['omega'])  # control moment

        x = state['x'] + state['x_dot'] * dt
        v = state['x_dot'] + \
            ((1/self.mass)*(self.mass*self.g*self.e3 - f*R@self.e3)) * dt
        orientation = R + (R @ hat(state['omega'])) * dt
        angular_velocity = state['omega'] + np.linalg.inv(self.inertia) @ (
            M - np.cross(state['omega'], self.inertia@state['omega'])) * dt
